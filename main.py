...
required.add_argument('-n', '--numberOfprocesses', help='Number of processes', required=True)
parser.parse_args()
...
if args.type == 'sequence':
    ...
    process = subprocess.Popen(f"mpiexec -n {args.numberOfprocesses} python -m mpi4py parallel_flow.py {args.dataset} {args.method}" , shell=True)
